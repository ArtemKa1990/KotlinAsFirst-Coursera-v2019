...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((name, number) in mapA) {
        if (!resultMap.containsKey(name)) {
            if (mapB.containsKey(name) && mapB.getValue(name) != number) {
                resultMap[name] = number + ", " + mapB.getValue(name)
            } else resultMap[name] = number
        }
    }
    ...
    for ((person, handshakes) in friends) {
        ...
        if (handshake.isNotEmpty()) {
            for (resHandshake in handshake) {
                ...
                resultMap[person] =
                    (resultMap[person] ?: setOf<String>()).plus(
                        (friends[resHandshake] ?: setOf<String>())
                            .plus(handshakes).minus(person)
                    )
            }
        } else {
            resultMap[person] = handshakes
        }
    }
    if (resultMap != friends) {
        resultMap = propagateHandshakes(resultMap) as MutableMap<String, Set<String>>
    }
    return resultMap
}
