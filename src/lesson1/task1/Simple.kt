...
fun thirdDigit(number: Int): Int {
    ...
    /* Первоначальное решение, которое мне хотелось переделать...
    var res = number % 10
    var res1 = ((number - res) / 10) % 10
    return ((((number - res) / 10) - res1) / 10) % 10*/
}
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    var res: Double = (initial * percent / 100.0) + initial
    res += (res * percent / 100)
    res += (res * percent / 100)
    return res
}
...
fun numberRevert(number: Int): Int {
    ...
    while (initialNumber != 0) {
        positionNumber = initialNumber % 10
        resultNumber = (resultNumber * 10) + positionNumber
        initialNumber = (initialNumber - positionNumber) / 10
    }
    return resultNumber
}
